-- ============================================================================
-- MIGRATION V4: Add Trading Accounts System (With Explicit Type Casts)
-- ============================================================================
-- Fixed: Added explicit type casts to avoid VARCHAR/UUID comparison issues
-- ============================================================================
...
-- Create policies with explicit type casts (VARCHAR::TEXT comparison)
CREATE POLICY "Users can view their own accounts"
  ON trade_accounts FOR SELECT
  USING (user_id::TEXT IN (
    SELECT id::TEXT FROM user_profiles WHERE supabase_user_id = auth.uid()
  ));
...
FROM trades t
WHERE NOT EXISTS (
  SELECT 1 FROM trade_accounts ta WHERE ta.user_id::TEXT = t.user_id::TEXT
)
GROUP BY t.user_id;
...
SET account_id = (
  ...
  WHERE ta.user_id::TEXT = t.user_id::TEXT
...
-- MIGRATION COMPLETE!
-- ============================================================================
-- ✅ Created trade_accounts table with explicit type casts in RLS policies
