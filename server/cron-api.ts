...
app.post('/cron/ff-high-impact', authenticate, async (req, res) => {
  try {
...
app.post('/cron/ff-full-refresh', authenticate, async (req, res) => {
  try {
...
app.post('/cron/hourly-update', authenticate, async (req, res) => {
  try {
...
app.post('/cron/weekly-analysis', authenticate, async (req, res) => {
  try {
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n🔄 Cron API Server running on port ${PORT}`);
  console.log(`🔐 API Key authentication enabled`);
  console.log(`📡 Endpoints available:`);
  console.log(`   POST /cron/ff-high-impact (every 15 min)`);
  console.log(`   POST /cron/ff-full-refresh (every 4 hours)`);
  console.log(`   POST /cron/hourly-update (every 30 min/hourly)`);
  console.log(`   POST /cron/weekly-analysis (Sunday 00:00 UTC)`);
