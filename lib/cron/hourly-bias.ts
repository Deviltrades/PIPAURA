...
import { getEconomicScores } from './forex-factory';
...
const CENTRAL_BANK_TONE: Record<string, string> = {
  USD: 'hawkish',
  EUR: 'neutral',
  GBP: 'neutral',
  JPY: 'dovish',
...
async function getYahooPrice(ticker: string, days = 7): Promise<[number, number] | null> {
  try {
...
async function getPercentChange(ticker: string): Promise<number> {
...
async function fetchMarkets(): Promise<MarketData> {
  ...
  return Object.fromEntries(results) as MarketData;
}

function scoreCBTone(tone: string): [number, string[]] {
  if (tone === 'hawkish') return [W_CB_TONE, ['CB: hawkish']];
  if (tone === 'dovish') return [-W_CB_TONE, ['CB: dovish']];
  return [0, []];
}

function scoreCommodities(ccy: string, mkt: MarketData): [number, string[]] {
  let s = 0;
  const notes: string[] = [];
...
function scoreMarketFlows(ccy: string, mkt: MarketData): [number, string[]] {
  let s = 0;
  const notes: string[] = [];
...
function indexBiasLabel(score: number): string {
...
export async function runHourlyUpdate(supabaseUrl: string, supabaseKey: string): Promise<void> {
  ...
  const perCcy: Record<string, any> = {};
  ...
  for (const [currency, ecoScore] of Object.entries(economicScores)) {
    if (perCcy[currency]) {
      perCcy[currency].total_score += ecoScore;
      perCcy[currency].data_score = ecoScore;
      if (ecoScore !== 0) {
        perCcy[currency].notes.push(`Economic data: ${ecoScore >= 0 ? '+' : ''}${ecoScore}`);
      }
    }
  }
  ...
  for (const row of pairRows) {
    await supabase.from('fundamental_bias').upsert(row);
  }
